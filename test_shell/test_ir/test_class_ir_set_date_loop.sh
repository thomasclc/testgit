...
##
#	\功能编号 	
# \功能说明	终端反复设置时间（由2810终端几率出现设置时间失败的问题）
#	\用例步骤	通过遥控器菜单发反复修改任务
#	\验证点		每次都能修改成功
##
_test_case_ir_set_date_loop()
{
	...
	send_key_and_wait "f3:down:right:up:up:up:right:down:down:down:down:down:del:del:0:ok:up:up"
	while [ 1 ] ; do
		debug "set time: 12:59:00 "
		send_key_and_wait "del:del:1:2:down:del:del:5:9:ok:up:wait3"
		...
		send_key_and_wait "del:del:1:down:del:del:1:ok:up:wait3"
